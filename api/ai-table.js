...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKeys.length) {
      console.error("Missing GEMINI_API_KEYS env variable");
      res.status(500).json({ code: "MISSING_GEMINI_KEYS", error: "No Gemini API key configured in Vercel Production Environment Variables" });
      return;
    }
    ...
    if (!result) {
      console.error("Gemini generation failed", { status: lastError?.status, message: lastError?.message });
